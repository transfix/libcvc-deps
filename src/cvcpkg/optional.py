...
worked example (``docs/haikuports-integration.md``): its ports tree has
click and pyyaml, but pins ``cryptography`` at 3.4.8 against a ``>=41``
floor, ships ``sqlalchemy`` 1.3.24 against a ``^2.0`` floor, and has no
``greenlet`` or ``httpx`` port at all — so four mandatory distributions the
client never imports were what stopped the install, not the code.
