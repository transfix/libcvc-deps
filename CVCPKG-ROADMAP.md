...
> **Archive format handling (proposed):** the container suffix (`.tar.zst` / `.tar.gz` / `.zip`) is decided by platform conditionals re-derived in ~5 places, and external consumers (CI publish lanes) hardcode globs to find packed archives — which silently no-op'd a successful Windows publish (glob `*.tar.zst` vs the `.zip` pack writes). Make the format a single source of truth, stop making consumers guess the extension (`publish --all` already self-resolves), and promote the format to an explicit `--format` choice with per-platform defaults. See [`docs/roadmap/archive-format-handling.md`](docs/roadmap/archive-format-handling.md).
