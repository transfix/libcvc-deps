# recipes/x264-cli/build.ps1 — build x264 encoder CLI on Windows via MSYS2/MinGW64.
...
$cmd = "$depsFlag cd '$msysSource' && ./configure --prefix='$msysPrefix' --host=x86_64-w64-mingw32 --cross-prefix=x86_64-w64-mingw32- --enable-cli --disable-shared --disable-static --disable-lavf --disable-swscale --disable-opencl && make -j $jobs x264 && mkdir -p '$msysPrefix/bin' && install -m 755 x264.exe '$msysPrefix/bin/x264.exe'"
Write-Host "cvcpkg: bash -lc `"$cmd`""
& $bash -lc $cmd
if ($LASTEXITCODE -ne 0) { throw 'x264-cli build failed' }
