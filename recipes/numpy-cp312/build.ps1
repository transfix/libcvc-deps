# recipes/numpy-cp312/build.ps1 — build NumPy FROM SOURCE (meson-python) against
# cvcpkg's OpenBLAS on Windows/MSVC, then install the wheel into the python312
...
# name and resolve it at import out of <prefix>\bin, which the python31X recipe's
# cvcpkg-dll-directories.pth adds via os.add_dll_directory at startup. That .pth
# is what replaces $ORIGIN here — without it every extension linking a prefix
# library fails with "DLL load failed".
...
Write-Output "numpy-cp312: building with $py"
...
if (-not $pkgconf) {
    throw ("numpy-cp312: no pkg-config in the dependency prefix. numpy's meson " +
...
if ($LASTEXITCODE -ne 0) {
    ...
    throw "numpy-cp312: openblas.pc not found by $pkgconf (PKG_CONFIG_LIBDIR=$pcPath)"
}
Write-Output "numpy-cp312: openblas $(& $env:PKG_CONFIG --modversion openblas) via $pkgconf"
...
if ($LASTEXITCODE -ne 0) {
    ...
    throw "numpy-cp312: pip wheel failed ($LASTEXITCODE)"
}

$wheel = Get-ChildItem -Path $wheelhouse -Filter 'numpy-*.whl' -File | Select-Object -First 1
if (-not $wheel) { throw "numpy-cp312: no wheel produced under $wheelhouse" }
Write-Output "numpy-cp312: built $($wheel.Name)"

& $py -m pip install --no-deps --no-index --no-compile --ignore-installed `
    --prefix $env:CVC_INSTALL_DIR $wheel.FullName
if ($LASTEXITCODE -ne 0) { throw "numpy-cp312: pip install failed ($LASTEXITCODE)" }
...
"@
if ($LASTEXITCODE -ne 0) { throw "numpy-cp312: verification failed ($LASTEXITCODE)" }
Write-Output "numpy-cp312 build + verification complete"
