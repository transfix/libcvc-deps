...
$py = Get-CvcPythonExe
Write-Output "tinycss2-cp313t: building with $py"
...
if ($LASTEXITCODE -ne 0) { throw "tinycss2-cp313t: pip wheel failed ($LASTEXITCODE)" }

$wheel = Get-ChildItem -Path $wheelhouse -Filter '*.whl' -File | Select-Object -First 1
if (-not $wheel) { throw "tinycss2-cp313t: no wheel produced under $wheelhouse" }
Write-Output "tinycss2-cp313t: built $($wheel.Name)"

& $py -m pip install --no-index --no-deps --no-compile --ignore-installed `
    --prefix $env:CVC_INSTALL_DIR $wheel.FullName
if ($LASTEXITCODE -ne 0) { throw "tinycss2-cp313t: pip install failed ($LASTEXITCODE)" }
