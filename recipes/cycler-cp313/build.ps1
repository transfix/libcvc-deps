# recipes/cycler-cp313/build.ps1 — build cycler 0.12.1 FROM SOURCE (generated).
...
$py = Get-CvcPythonExe
Write-Output "cycler-cp313: building with $py"
...
if ($LASTEXITCODE -ne 0) { throw "cycler-cp313: pip wheel failed ($LASTEXITCODE)" }

$wheel = Get-ChildItem -Path $wheelhouse -Filter '*.whl' -File | Select-Object -First 1
if (-not $wheel) { throw "cycler-cp313: no wheel produced under $wheelhouse" }
Write-Output "cycler-cp313: built $($wheel.Name)"

& $py -m pip install --no-index --no-deps --no-compile --ignore-installed `
    --prefix $env:CVC_INSTALL_DIR $wheel.FullName
if ($LASTEXITCODE -ne 0) { throw "cycler-cp313: pip install failed ($LASTEXITCODE)" }

# The check must exercise the runtime closure, not the build-only backend.
$env:PYTHONPATH = ''
Invoke-CvcPythonCheck 'import cycler'
