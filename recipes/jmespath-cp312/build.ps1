# recipes/jmespath-cp312/build.ps1 — build jmespath 1.1.0 FROM SOURCE (generated).
...
$py = Get-CvcPythonExe
Write-Output "jmespath-cp312: building with $py"
...
if ($LASTEXITCODE -ne 0) { throw "jmespath-cp312: pip wheel failed ($LASTEXITCODE)" }

$wheel = Get-ChildItem -Path $wheelhouse -Filter '*.whl' -File | Select-Object -First 1
if (-not $wheel) { throw "jmespath-cp312: no wheel produced under $wheelhouse" }
Write-Output "jmespath-cp312: built $($wheel.Name)"

& $py -m pip install --no-index --no-deps --no-compile --ignore-installed `
    --prefix $env:CVC_INSTALL_DIR $wheel.FullName
if ($LASTEXITCODE -ne 0) { throw "jmespath-cp312: pip install failed ($LASTEXITCODE)" }
