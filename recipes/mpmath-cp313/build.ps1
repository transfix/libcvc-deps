# recipes/mpmath-cp313/build.ps1 — build mpmath 1.3.0 FROM SOURCE (generated).
...
$py = Get-CvcPythonExe
Write-Output "mpmath-cp313: building with $py"
...
if ($LASTEXITCODE -ne 0) { throw "mpmath-cp313: pip wheel failed ($LASTEXITCODE)" }

$wheel = Get-ChildItem -Path $wheelhouse -Filter '*.whl' -File | Select-Object -First 1
if (-not $wheel) { throw "mpmath-cp313: no wheel produced under $wheelhouse" }
Write-Output "mpmath-cp313: built $($wheel.Name)"

& $py -m pip install --no-index --no-deps --no-compile --ignore-installed `
    --prefix $env:CVC_INSTALL_DIR $wheel.FullName
if ($LASTEXITCODE -ne 0) { throw "mpmath-cp313: pip install failed ($LASTEXITCODE)" }
