# recipes/boto3-cp311/build.ps1 — build boto3 1.43.49 FROM SOURCE (generated).
...
$py = Get-CvcPythonExe
Write-Output "boto3-cp311: building with $py"
...
if ($LASTEXITCODE -ne 0) { throw "boto3-cp311: pip wheel failed ($LASTEXITCODE)" }

$wheel = Get-ChildItem -Path $wheelhouse -Filter '*.whl' -File | Select-Object -First 1
if (-not $wheel) { throw "boto3-cp311: no wheel produced under $wheelhouse" }
Write-Output "boto3-cp311: built $($wheel.Name)"

& $py -m pip install --no-index --no-deps --no-compile --ignore-installed `
    --prefix $env:CVC_INSTALL_DIR $wheel.FullName
if ($LASTEXITCODE -ne 0) { throw "boto3-cp311: pip install failed ($LASTEXITCODE)" }

# The check must exercise the runtime closure, not the build-only backend.
$env:PYTHONPATH = ''
Invoke-CvcPythonCheck 'import boto3'
