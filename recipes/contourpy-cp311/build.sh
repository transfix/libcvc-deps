...
#
# BUILD BACKEND: meson, meson-python-cp311, pybind11-cp311
...
echo "contourpy-cp311: building with ${PY_EXE}"
...
[ -d "${_PB11_INC}" ] || { echo "contourpy-cp311: pybind11.get_include() -> ${_PB11_INC} does not exist" >&2; exit 1; }
export CXXFLAGS="-I${_PB11_INC} ${CXXFLAGS:-}"
echo "contourpy-cp311: pybind11 pc=${_PB11_PC} include=${_PB11_INC} cflags=[$(pkg-config --cflags pybind11 2>&1 || true)]"
...
[ -n "${WHEEL}" ] || { echo "contourpy-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "contourpy-cp311: built $(basename "${WHEEL}")"
