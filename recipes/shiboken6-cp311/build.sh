...
# the importable cp311 `shiboken6` package (wrapInstance), and the Shiboken6
...
# are built against the SAME cp311 the host embeds.
PY_EXE=""
...
if [[ -z "${PY_EXE}" ]]; then
    echo "shiboken6: could not find python3.11 in the dependency closure" >&2
    exit 1
fi
...
# python311 interpreter's own prefix). shiboken otherwise prefixes
...
# Per-interpreter column: the built shiboken6 tree stays in python3.11's
