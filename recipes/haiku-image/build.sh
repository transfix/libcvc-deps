...
quit
BFSCMDS
        # Read the key back out of the BFS volume and compare.
        rm -f "${INJ}/authorized_keys.readback"
        sudo "${BFS_SHELL}" "${BFSPART}" <<BFSREAD >/dev/null 2>&1 || true
cp home/.ssh/authorized_keys :${INJ}/authorized_keys.readback
quit
BFSREAD
        if [[ -n "${HAIKU_BUILDER_SSH_PUBKEY:-}" ]] \
           && cmp -s "${INJ}/authorized_keys" "${INJ}/authorized_keys.readback" 2>/dev/null; then
            SSH_PUBKEY_BAKED=true
            echo "SSH key injection confirmed by read-back."
        elif [[ -n "${HAIKU_BUILDER_SSH_PUBKEY:-}" ]]; then
            echo "WARN: SSH key read-back did not match; the image is NOT key-trusting." >&2
            echo "      image.yaml will say ssh_pubkey_baked: false." >&2
        fi
