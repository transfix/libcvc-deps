...
$oggFlag    = if ($msysDeps) { "--with-ogg='$msysDeps'" } else { '' }

$cmd = "$depsFlag cd '$msysSource' && ./configure --prefix='$msysPrefix' --host=x86_64-w64-mingw32 $sharedFlag --disable-dependency-tracking $oggFlag && make -j $jobs && make install"
