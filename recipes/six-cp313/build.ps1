# recipes/six-cp313/build.ps1 — build six 1.17.0 FROM SOURCE (generated).
...
$py = Get-CvcPythonExe
Write-Output "six-cp313: building with $py"
...
if ($LASTEXITCODE -ne 0) { throw "six-cp313: pip wheel failed ($LASTEXITCODE)" }

$wheel = Get-ChildItem -Path $wheelhouse -Filter '*.whl' -File | Select-Object -First 1
if (-not $wheel) { throw "six-cp313: no wheel produced under $wheelhouse" }
Write-Output "six-cp313: built $($wheel.Name)"

& $py -m pip install --no-index --no-deps --no-compile --ignore-installed `
    --prefix $env:CVC_INSTALL_DIR $wheel.FullName
if ($LASTEXITCODE -ne 0) { throw "six-cp313: pip install failed ($LASTEXITCODE)" }

# The check must exercise the runtime closure, not the build-only backend.
$env:PYTHONPATH = ''
Invoke-CvcPythonCheck 'import six'
