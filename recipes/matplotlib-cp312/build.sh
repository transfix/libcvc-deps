#!/usr/bin/env bash
# recipes/matplotlib-cp312/build.sh — build matplotlib 3.10.0 FROM SOURCE (generated).
#
...
#
# BUILD BACKEND: meson-python-cp312, pybind11-cp312, setuptools-scm-cp312
...
echo "matplotlib-cp312: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "matplotlib-cp312: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "matplotlib-cp312: built $(basename "${WHEEL}")"
...
unset PYTHONPATH
cvc_python_check "import matplotlib; matplotlib.use('Agg'); import matplotlib.pyplot"
