...
[ -n "${MPL_DIR}" ] || { echo "matplotlib-cp312: staged matplotlib/ not found" >&2; exit 1; }

# ── Relocatable RUNPATH per-file (extensions at MIXED depths) ───────────────
if [ "${CVC_PLATFORM}" != "macos" ]; then
  command -v patchelf >/dev/null 2>&1 || { echo "matplotlib-cp312: patchelf missing" >&2; exit 1; }
...
print("matplotlib", matplotlib.__version__,
      ...
      "| render OK")
PYCHECK
echo "matplotlib-cp312 build + verification complete"
