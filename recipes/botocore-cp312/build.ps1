# recipes/botocore-cp312/build.ps1 — build botocore 1.43.49 FROM SOURCE (generated).
...
$py = Get-CvcPythonExe
Write-Output "botocore-cp312: building with $py"
...
if ($LASTEXITCODE -ne 0) { throw "botocore-cp312: pip wheel failed ($LASTEXITCODE)" }

$wheel = Get-ChildItem -Path $wheelhouse -Filter '*.whl' -File | Select-Object -First 1
if (-not $wheel) { throw "botocore-cp312: no wheel produced under $wheelhouse" }
Write-Output "botocore-cp312: built $($wheel.Name)"

& $py -m pip install --no-index --no-deps --no-compile --ignore-installed `
    --prefix $env:CVC_INSTALL_DIR $wheel.FullName
if ($LASTEXITCODE -ne 0) { throw "botocore-cp312: pip install failed ($LASTEXITCODE)" }
