#!/usr/bin/env bash
# recipes/matplotlib-cp313/build.sh — build matplotlib 3.10.0 FROM SOURCE
...
#     as recipes/contourpy-cp313: the .pc dir via --pkgconfigdir plus the
...
echo "matplotlib-cp313: building with ${PY_EXE}"
...
[ -n "${_PB11_PC}" ] || { echo "matplotlib-cp313: python -m pybind11 --pkgconfigdir returned nothing" >&2; exit 1; }
_PB11_INC="$("${PY_EXE}" -c 'import pybind11; print(pybind11.get_include())')"
[ -d "${_PB11_INC}" ] || { echo "matplotlib-cp313: pybind11.get_include() -> ${_PB11_INC} does not exist" >&2; exit 1; }
...
[ -n "${PKG_CONFIG:-}" ] || { echo "matplotlib-cp313: no pkg-config in ${BLD}/bin, ${DEPS}/bin or PATH" >&2; exit 1; }
...
for _mod in freetype2 qhull_r; do
    if ! "${PKG_CONFIG}" --exists "${_mod}"; then
        echo "matplotlib-cp313: ${_mod}.pc not found by ${PKG_CONFIG}" >&2
...
echo "matplotlib-cp313: pybind11 pc=${_PB11_PC} include=${_PB11_INC}; freetype $("${PKG_CONFIG}" --modversion freetype2), qhull_r $("${PKG_CONFIG}" --modversion qhull_r)"
...
[ -n "${WHEEL}" ] || { echo "matplotlib-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
echo "matplotlib-cp313: built $(basename "${WHEEL}")"
...
[ -n "${MPL_DIR}" ] || { echo "matplotlib-cp313: staged matplotlib/ not found" >&2; exit 1; }

# ── Relocatable RUNPATH per-file (extensions at MIXED depths) ───────────────
if [ "${CVC_PLATFORM}" != "macos" ]; then
  command -v patchelf >/dev/null 2>&1 || { echo "matplotlib-cp313: patchelf missing" >&2; exit 1; }
...
print("matplotlib", matplotlib.__version__,
      ...
      "| render OK")
PYCHECK
echo "matplotlib-cp313 build + verification complete"
