# recipes/markupsafe-cp311/build.ps1 — Windows from-source build of MarkupSafe 3.0.3 for
...
Write-Output "markupsafe-cp311: building with $py"
...
if ($LASTEXITCODE -ne 0) { throw "markupsafe-cp311: setuptools not importable from the build prefix" }
...
& $py @pipArgs
if ($LASTEXITCODE -ne 0) { throw "markupsafe-cp311: pip wheel failed ($LASTEXITCODE)" }

$wheel = Get-ChildItem -Path $wheelhouse -Filter '*.whl' -File | Select-Object -First 1
if (-not $wheel) { throw "markupsafe-cp311: no wheel produced under $wheelhouse" }
Write-Output "markupsafe-cp311: built $($wheel.Name)"
...
if ($LASTEXITCODE -ne 0) { throw "markupsafe-cp311: pip install failed ($LASTEXITCODE)" }

$sitePackages = Join-Path $env:CVC_INSTALL_DIR 'Lib\site-packages'
if (-not (Test-Path -LiteralPath $sitePackages)) {
    throw "markupsafe-cp311: no Lib\site-packages under $env:CVC_INSTALL_DIR after pip install"
}
Write-Output "markupsafe-cp311: staged into $sitePackages"
...
'@
& $py -c $check
if ($LASTEXITCODE -ne 0) { throw "markupsafe-cp311: verification failed ($LASTEXITCODE)" }

Write-Output "markupsafe-cp311: build + verification complete"
