# recipes/cffi-cp312/build.ps1 — Windows from-source build of cffi 2.0.0 for
...
Write-Output "cffi-cp312: building with $py"
...
if ($LASTEXITCODE -ne 0) { throw "cffi-cp312: setuptools not importable from the build prefix" }
...
& $py @pipArgs
if ($LASTEXITCODE -ne 0) { throw "cffi-cp312: pip wheel failed ($LASTEXITCODE)" }

$wheel = Get-ChildItem -Path $wheelhouse -Filter '*.whl' -File | Select-Object -First 1
if (-not $wheel) { throw "cffi-cp312: no wheel produced under $wheelhouse" }
Write-Output "cffi-cp312: built $($wheel.Name)"
...
if ($LASTEXITCODE -ne 0) { throw "cffi-cp312: pip install failed ($LASTEXITCODE)" }

$sitePackages = Join-Path $env:CVC_INSTALL_DIR 'Lib\site-packages'
if (-not (Test-Path -LiteralPath $sitePackages)) {
    throw "cffi-cp312: no Lib\site-packages under $env:CVC_INSTALL_DIR after pip install"
}
Write-Output "cffi-cp312: staged into $sitePackages"
...
'@
& $py -c $check
if ($LASTEXITCODE -ne 0) { throw "cffi-cp312: verification failed ($LASTEXITCODE)" }

Write-Output "cffi-cp312: build + verification complete"
