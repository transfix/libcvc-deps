# recipes/aiosqlite-cp313/build.ps1 — build aiosqlite 0.22.1 FROM SOURCE (generated).
...
$py = Get-CvcPythonExe
Write-Output "aiosqlite-cp313: building with $py"
...
if ($LASTEXITCODE -ne 0) { throw "aiosqlite-cp313: pip wheel failed ($LASTEXITCODE)" }

$wheel = Get-ChildItem -Path $wheelhouse -Filter '*.whl' -File | Select-Object -First 1
if (-not $wheel) { throw "aiosqlite-cp313: no wheel produced under $wheelhouse" }
Write-Output "aiosqlite-cp313: built $($wheel.Name)"

& $py -m pip install --no-index --no-deps --no-compile --ignore-installed `
    --prefix $env:CVC_INSTALL_DIR $wheel.FullName
if ($LASTEXITCODE -ne 0) { throw "aiosqlite-cp313: pip install failed ($LASTEXITCODE)" }
