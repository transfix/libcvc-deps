...
# Locate the cvcpkg python311 interpreter in the dependency closure so the
# wrappers are built against the SAME Python (cp311) that pycvc / pycvc-gl use.
PY_EXE=""
...
if [[ -z "${PY_EXE}" ]]; then
    echo "vtk-python: could not find python3.11 in the dependency closure" >&2
    exit 1
fi
...
cmake -G Ninja \
    ...
    -DVTK_PYTHON_SITE_PACKAGES_SUFFIX="lib/python3.11/site-packages" \
