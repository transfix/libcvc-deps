# recipes/googleapis-common-protos-cp311/build.ps1 — build googleapis-common-protos 1.75.0 FROM SOURCE (generated).
...
$py = Get-CvcPythonExe
Write-Output "googleapis-common-protos-cp311: building with $py"
...
if ($LASTEXITCODE -ne 0) { throw "googleapis-common-protos-cp311: pip wheel failed ($LASTEXITCODE)" }

$wheel = Get-ChildItem -Path $wheelhouse -Filter '*.whl' -File | Select-Object -First 1
if (-not $wheel) { throw "googleapis-common-protos-cp311: no wheel produced under $wheelhouse" }
Write-Output "googleapis-common-protos-cp311: built $($wheel.Name)"

& $py -m pip install --no-index --no-deps --no-compile --ignore-installed `
    --prefix $env:CVC_INSTALL_DIR $wheel.FullName
if ($LASTEXITCODE -ne 0) { throw "googleapis-common-protos-cp311: pip install failed ($LASTEXITCODE)" }
