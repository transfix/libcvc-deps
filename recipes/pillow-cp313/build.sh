#!/usr/bin/env bash
# recipes/pillow-cp313/build.sh — build pillow 11.1.0 FROM SOURCE (generated).
#
...
echo "pillow-cp313: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "pillow-cp313: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
