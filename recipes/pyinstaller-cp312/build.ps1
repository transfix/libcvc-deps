# recipes/pyinstaller-cp312/build.ps1 — Windows from-source build of PyInstaller 6.21.0.
#
...
if (-not $env:CVC_PYTHON_ABI) { $env:CVC_PYTHON_ABI = "cp312" }
$pyExe = Get-CvcPythonExe
Write-Host "pyinstaller-cp312: building against $pyExe"
...
if ($LASTEXITCODE -ne 0) { throw "pyinstaller-cp312: pip wheel failed" }

$wheel = Get-ChildItem -Path $wheelhouse -Filter "pyinstaller-*.whl" | Select-Object -First 1
if (-not $wheel) { throw "pyinstaller-cp312: no wheel produced under $wheelhouse" }
Write-Host "pyinstaller-cp312: built $($wheel.Name)"

& $pyExe -m pip install --no-index --no-deps --no-compile `
    --prefix $env:CVC_INSTALL_DIR $wheel.FullName
if ($LASTEXITCODE -ne 0) { throw "pyinstaller-cp312: pip install failed" }

Invoke-CvcPythonCheck "import PyInstaller; print('PyInstaller', PyInstaller.__version__)"

Write-Host "pyinstaller-cp312: build + verification complete"
