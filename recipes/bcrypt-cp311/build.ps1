# recipes/bcrypt-cp311/build.ps1 — Windows from-source build of bcrypt 5.0.0 for
...
Write-Output "bcrypt-cp311: building with $py"
...
if ($LASTEXITCODE -ne 0) { throw "bcrypt-cp311: build backend not importable from the build prefix" }
...
function Resolve-CvcPrefixTool([string]$Name) {
    ...
    if (-not $cmd) { throw "bcrypt-cp311: $Name not on PATH — is the 'rust' build dep in the closure?" }
    $path = $cmd.Source
    if (-not ($path.StartsWith($bld, 'OrdinalIgnoreCase') -or $path.StartsWith($deps, 'OrdinalIgnoreCase'))) {
        throw "bcrypt-cp311: refusing $Name at $path — outside the cvcpkg prefixes ($bld, $deps)"
    }
    return $path
}
...
if ($env:CVC_CARGO_OFFLINE) {
    ...
    Write-Output "bcrypt-cp311: CARGO_NET_OFFLINE=true — $env:CARGO_HOME must already hold every crate in Cargo.lock"
} else {
    $env:CARGO_NET_OFFLINE = 'false'
    Write-Output "bcrypt-cp311: NOTE - cargo will FETCH crates.io (sdist ships Cargo.lock, no vendored sources)"
}
...
& $py @pipArgs
if ($LASTEXITCODE -ne 0) { throw "bcrypt-cp311: pip wheel failed ($LASTEXITCODE)" }

$wheel = Get-ChildItem -Path $wheelhouse -Filter '*.whl' -File | Select-Object -First 1
if (-not $wheel) { throw "bcrypt-cp311: no wheel produced under $wheelhouse" }
Write-Output "bcrypt-cp311: built $($wheel.Name)"

& $py -m pip install --no-index --no-deps --no-compile --prefix $env:CVC_INSTALL_DIR $wheel.FullName
if ($LASTEXITCODE -ne 0) { throw "bcrypt-cp311: pip install failed ($LASTEXITCODE)" }

$sitePackages = Join-Path $env:CVC_INSTALL_DIR 'Lib\site-packages'
if (-not (Test-Path -LiteralPath $sitePackages)) {
    throw "bcrypt-cp311: no Lib\site-packages under $env:CVC_INSTALL_DIR after pip install"
}
Write-Output "bcrypt-cp311: staged into $sitePackages"
...
'@
& $py -c $check
if ($LASTEXITCODE -ne 0) { throw "bcrypt-cp311: verification failed ($LASTEXITCODE)" }

Write-Output "bcrypt-cp311: build + verification complete"
