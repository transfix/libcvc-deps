# recipes/farama-notifications-cp311/build.ps1 — build farama-notifications 0.0.6 FROM SOURCE (generated).
...
$py = Get-CvcPythonExe
Write-Output "farama-notifications-cp311: building with $py"
...
if ($LASTEXITCODE -ne 0) { throw "farama-notifications-cp311: pip wheel failed ($LASTEXITCODE)" }

$wheel = Get-ChildItem -Path $wheelhouse -Filter '*.whl' -File | Select-Object -First 1
if (-not $wheel) { throw "farama-notifications-cp311: no wheel produced under $wheelhouse" }
Write-Output "farama-notifications-cp311: built $($wheel.Name)"

& $py -m pip install --no-index --no-deps --no-compile --ignore-installed `
    --prefix $env:CVC_INSTALL_DIR $wheel.FullName
if ($LASTEXITCODE -ne 0) { throw "farama-notifications-cp311: pip install failed ($LASTEXITCODE)" }
