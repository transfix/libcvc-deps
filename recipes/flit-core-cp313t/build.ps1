# recipes/flit-core-cp313t/build.ps1 — build flit-core 3.12.0 FROM SOURCE (generated).
...
$py = Get-CvcPythonExe
Write-Output "flit-core-cp313t: building with $py"
...
if ($LASTEXITCODE -ne 0) { throw "flit-core-cp313t: pip wheel failed ($LASTEXITCODE)" }

$wheel = Get-ChildItem -Path $wheelhouse -Filter '*.whl' -File | Select-Object -First 1
if (-not $wheel) { throw "flit-core-cp313t: no wheel produced under $wheelhouse" }
Write-Output "flit-core-cp313t: built $($wheel.Name)"

& $py -m pip install --no-index --no-deps --no-compile --ignore-installed `
    --prefix $env:CVC_INSTALL_DIR $wheel.FullName
if ($LASTEXITCODE -ne 0) { throw "flit-core-cp313t: pip install failed ($LASTEXITCODE)" }

# The check must exercise the runtime closure, not the build-only backend.
$env:PYTHONPATH = ''
Invoke-CvcPythonCheck 'import flit_core'
