# recipes/h11-cp312/build.ps1 — build h11 0.16.0 FROM SOURCE (generated).
...
$py = Get-CvcPythonExe
Write-Output "h11-cp312: building with $py"
...
if ($LASTEXITCODE -ne 0) { throw "h11-cp312: pip wheel failed ($LASTEXITCODE)" }

$wheel = Get-ChildItem -Path $wheelhouse -Filter '*.whl' -File | Select-Object -First 1
if (-not $wheel) { throw "h11-cp312: no wheel produced under $wheelhouse" }
Write-Output "h11-cp312: built $($wheel.Name)"

& $py -m pip install --no-index --no-deps --no-compile --ignore-installed `
    --prefix $env:CVC_INSTALL_DIR $wheel.FullName
if ($LASTEXITCODE -ne 0) { throw "h11-cp312: pip install failed ($LASTEXITCODE)" }

# The check must exercise the runtime closure, not the build-only backend.
$env:PYTHONPATH = ''
Invoke-CvcPythonCheck 'import h11'
