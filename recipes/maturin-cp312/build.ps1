# recipes/maturin-cp312/build.ps1 — Windows from-source build of maturin 1.14.1
# for the cp312 interpreter column.
#
...
Write-Output "maturin-cp312: building with $py"
...
if ($LASTEXITCODE -ne 0) { throw "maturin-cp312: build backend not importable from the build prefix" }
...
function Resolve-CvcPrefixTool([string]$Name) {
    ...
    if (-not $cmd) { throw "maturin-cp312: $Name not on PATH — is the 'rust' build dep in the closure?" }
    $path = $cmd.Source
    if (-not ($path.StartsWith($bld, 'OrdinalIgnoreCase') -or $path.StartsWith($deps, 'OrdinalIgnoreCase'))) {
        throw "maturin-cp312: refusing $Name at $path — outside the cvcpkg prefixes ($bld, $deps)"
    }
    return $path
}
...
if ($env:CVC_CARGO_OFFLINE) {
    ...
    Write-Output "maturin-cp312: CARGO_NET_OFFLINE=true — $env:CARGO_HOME must already hold every crate in Cargo.lock"
} else {
    $env:CARGO_NET_OFFLINE = 'false'
    Write-Output "maturin-cp312: NOTE - cargo will FETCH crates.io (sdist ships Cargo.lock, no vendored sources)"
}
...
& $py @pipArgs
if ($LASTEXITCODE -ne 0) { throw "maturin-cp312: pip wheel failed ($LASTEXITCODE)" }

$wheel = Get-ChildItem -Path $wheelhouse -Filter '*.whl' -File | Select-Object -First 1
if (-not $wheel) { throw "maturin-cp312: no wheel produced under $wheelhouse" }
Write-Output "maturin-cp312: built $($wheel.Name)"

& $py -m pip install --no-index --no-deps --no-compile --prefix $env:CVC_INSTALL_DIR $wheel.FullName
if ($LASTEXITCODE -ne 0) { throw "maturin-cp312: pip install failed ($LASTEXITCODE)" }

$sitePackages = Join-Path $env:CVC_INSTALL_DIR 'Lib\site-packages'
if (-not (Test-Path -LiteralPath $sitePackages)) {
    throw "maturin-cp312: no Lib\site-packages under $env:CVC_INSTALL_DIR after pip install"
}
Write-Output "maturin-cp312: staged into $sitePackages"

$maturinExe = Join-Path $env:CVC_INSTALL_DIR 'Scripts\maturin.exe'
if (-not (Test-Path -LiteralPath $maturinExe)) {
    throw "maturin-cp312: no maturin.exe staged at $maturinExe"
}
& $maturinExe --version
if ($LASTEXITCODE -ne 0) { throw "maturin-cp312: staged maturin.exe failed to run" }
...
'@
& $py -c $check
if ($LASTEXITCODE -ne 0) { throw "maturin-cp312: verification failed ($LASTEXITCODE)" }

Write-Output "maturin-cp312: build + verification complete"
