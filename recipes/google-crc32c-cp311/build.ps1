# recipes/google-crc32c-cp311/build.ps1 — build google-crc32c 1.8.0 FROM SOURCE (generated).
...
$py = Get-CvcPythonExe
Write-Output "google-crc32c-cp311: building with $py"
...
if ($LASTEXITCODE -ne 0) { throw "google-crc32c-cp311: pip wheel failed ($LASTEXITCODE)" }

$wheel = Get-ChildItem -Path $wheelhouse -Filter '*.whl' -File | Select-Object -First 1
if (-not $wheel) { throw "google-crc32c-cp311: no wheel produced under $wheelhouse" }
Write-Output "google-crc32c-cp311: built $($wheel.Name)"

& $py -m pip install --no-index --no-deps --no-compile --ignore-installed `
    --prefix $env:CVC_INSTALL_DIR $wheel.FullName
if ($LASTEXITCODE -ne 0) { throw "google-crc32c-cp311: pip install failed ($LASTEXITCODE)" }
