# recipes/numpy-cp313/build.ps1 — Windows: DEFERRED (loud stub).
#
...
# via os.add_dll_directory) is a tracked fast-follow; until it lands, numpy-cp313
# does not build on Windows. Fail loudly rather than silently ship nothing.
Set-StrictMode -Version Latest
$ErrorActionPreference = 'Stop'
Write-Error "numpy-cp313: Windows from-source build not yet implemented (see build.sh; MSVC/meson/OpenBLAS port pending). Linux/macOS/BSD build from build.sh."
exit 1
