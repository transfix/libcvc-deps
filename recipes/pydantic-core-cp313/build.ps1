# recipes/pydantic-core-cp313/build.ps1 — Windows from-source build of
# pydantic-core 2.46.4 for the cp313 interpreter column.
#
...
Write-Output "pydantic-core-cp313: building with $py"
...
if ($LASTEXITCODE -ne 0) { throw "pydantic-core-cp313: maturin backend not importable from the build prefix" }

$maturinCmd = Get-Command maturin -ErrorAction SilentlyContinue
if (-not $maturinCmd) { throw "pydantic-core-cp313: no 'maturin' executable on PATH (maturin-cp313 supplies it in $bld\Scripts)" }
& $maturinCmd.Source --version
if ($LASTEXITCODE -ne 0) { throw "pydantic-core-cp313: staged maturin failed to run" }
...
function Resolve-CvcPrefixTool([string]$Name) {
    ...
    if (-not $cmd) { throw "pydantic-core-cp313: $Name not on PATH — is the 'rust' build dep in the closure?" }
    $path = $cmd.Source
    if (-not ($path.StartsWith($bld, 'OrdinalIgnoreCase') -or $path.StartsWith($deps, 'OrdinalIgnoreCase'))) {
        throw "pydantic-core-cp313: refusing $Name at $path — outside the cvcpkg prefixes ($bld, $deps)"
    }
    return $path
}
...
if ($env:CVC_CARGO_OFFLINE) {
    ...
    Write-Output "pydantic-core-cp313: CARGO_NET_OFFLINE=true — $env:CARGO_HOME must already hold every crate in Cargo.lock"
} else {
    $env:CARGO_NET_OFFLINE = 'false'
    Write-Output "pydantic-core-cp313: NOTE - cargo will FETCH crates.io (sdist ships Cargo.lock, no vendored sources)"
}
...
& $py @pipArgs
if ($LASTEXITCODE -ne 0) { throw "pydantic-core-cp313: pip wheel failed ($LASTEXITCODE)" }

$wheel = Get-ChildItem -Path $wheelhouse -Filter '*.whl' -File | Select-Object -First 1
if (-not $wheel) { throw "pydantic-core-cp313: no wheel produced under $wheelhouse" }
Write-Output "pydantic-core-cp313: built $($wheel.Name)"

& $py -m pip install --no-index --no-deps --no-compile --prefix $env:CVC_INSTALL_DIR $wheel.FullName
if ($LASTEXITCODE -ne 0) { throw "pydantic-core-cp313: pip install failed ($LASTEXITCODE)" }

$sitePackages = Join-Path $env:CVC_INSTALL_DIR 'Lib\site-packages'
if (-not (Test-Path -LiteralPath $sitePackages)) {
    throw "pydantic-core-cp313: no Lib\site-packages under $env:CVC_INSTALL_DIR after pip install"
}
Write-Output "pydantic-core-cp313: staged into $sitePackages"
...
'@
& $py -c $check
if ($LASTEXITCODE -ne 0) { throw "pydantic-core-cp313: verification failed ($LASTEXITCODE)" }

Write-Output "pydantic-core-cp313: build + verification complete"
