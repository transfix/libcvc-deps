#!/usr/bin/env bash
# recipes/libxscrnsaver/build.sh — build libXScrnSaver 1.2.5 with autotools.
set -euo pipefail
...
# path, gone after cvcpkg relocates the prefix, so a downstream libtool link
# fails. The .so + pkg-config .pc carry the real link info.
