# recipes/jsonschema-cp311/build.ps1 — build jsonschema 4.17.3 FROM SOURCE (generated).
...
$py = Get-CvcPythonExe
Write-Output "jsonschema-cp311: building with $py"
...
if ($LASTEXITCODE -ne 0) { throw "jsonschema-cp311: pip wheel failed ($LASTEXITCODE)" }

$wheel = Get-ChildItem -Path $wheelhouse -Filter '*.whl' -File | Select-Object -First 1
if (-not $wheel) { throw "jsonschema-cp311: no wheel produced under $wheelhouse" }
Write-Output "jsonschema-cp311: built $($wheel.Name)"

& $py -m pip install --no-index --no-deps --no-compile --ignore-installed `
    --prefix $env:CVC_INSTALL_DIR $wheel.FullName
if ($LASTEXITCODE -ne 0) { throw "jsonschema-cp311: pip install failed ($LASTEXITCODE)" }
