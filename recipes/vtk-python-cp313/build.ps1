...
# Locate the cvcpkg python313 interpreter (cp313) in the dependency closure so the
...
foreach ($root in @($env:CVC_DEPS_PREFIX, $env:CVC_BUILD_PREFIX, $env:CVC_INSTALL_DIR)) {
    ...
    foreach ($cand in @("$root\python.exe", "$root\bin\python.exe", "$root\python3.13.exe")) {
        if (Test-Path $cand) { $pyExe = $cand; break }
    }
    if ($pyExe) { break }
}
if (-not $pyExe) { throw "vtk-python: could not find python (3.13) in the dependency closure" }
