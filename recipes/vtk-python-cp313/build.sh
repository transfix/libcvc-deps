...
# Locate the cvcpkg python313 interpreter in the dependency closure so the
# wrappers are built against the SAME Python (cp313) that pycvc / pycvc-gl use.
PY_EXE=""
...
if [[ -z "${PY_EXE}" ]]; then
    echo "vtk-python: could not find python3.13 in the dependency closure" >&2
    exit 1
fi
...
cmake -G Ninja \
    ...
    -DVTK_PYTHON_SITE_PACKAGES_SUFFIX="lib/python3.13/site-packages" \
