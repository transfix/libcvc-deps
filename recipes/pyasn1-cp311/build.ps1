# recipes/pyasn1-cp311/build.ps1 — build pyasn1 0.6.4 FROM SOURCE (generated).
...
$py = Get-CvcPythonExe
Write-Output "pyasn1-cp311: building with $py"
...
if ($LASTEXITCODE -ne 0) { throw "pyasn1-cp311: pip wheel failed ($LASTEXITCODE)" }

$wheel = Get-ChildItem -Path $wheelhouse -Filter '*.whl' -File | Select-Object -First 1
if (-not $wheel) { throw "pyasn1-cp311: no wheel produced under $wheelhouse" }
Write-Output "pyasn1-cp311: built $($wheel.Name)"

& $py -m pip install --no-index --no-deps --no-compile --ignore-installed `
    --prefix $env:CVC_INSTALL_DIR $wheel.FullName
if ($LASTEXITCODE -ne 0) { throw "pyasn1-cp311: pip install failed ($LASTEXITCODE)" }

# The check must exercise the runtime closure, not the build-only backend.
$env:PYTHONPATH = ''
Invoke-CvcPythonCheck 'import pyasn1'
