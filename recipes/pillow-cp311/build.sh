#!/usr/bin/env bash
# recipes/pillow-cp311/build.sh — build Pillow 11.1.0 FROM SOURCE against
# cvcpkg's zlib / libjpeg-turbo / libtiff / freetype / libwebp, then install
# the wheel into the python311 interpreter's site-packages.
#
...
[ -n "${PKG_CONFIG:-}" ] || { echo "pillow-cp311: no pkg-config in ${BLD}/bin, ${DEPS}/bin or PATH" >&2; exit 1; }
...
for _mod in zlib libjpeg libtiff-4 freetype2 libwebp; do
    if ! "${PKG_CONFIG}" --exists "${_mod}"; then
        echo "pillow-cp311: ${_mod}.pc not found by ${PKG_CONFIG}" >&2
        echo "  PKG_CONFIG_LIBDIR=${PKG_CONFIG_LIBDIR}" >&2
        ls -la "${DEPS}/lib/pkgconfig" 2>&1 | head -40 >&2
        exit 1
    fi
done
echo "pillow-cp311: zlib $("${PKG_CONFIG}" --modversion zlib), libjpeg $("${PKG_CONFIG}" --modversion libjpeg), libtiff $("${PKG_CONFIG}" --modversion libtiff-4), freetype $("${PKG_CONFIG}" --modversion freetype2), libwebp $("${PKG_CONFIG}" --modversion libwebp)"
...
[ -n "${WHEEL}" ] || { echo "pillow-cp311: no wheel produced" >&2; exit 1; }
echo "pillow-cp311: built $(basename "${WHEEL}")"
...
[ -n "${PIL_DIR}" ] || { echo "pillow-cp311: staged PIL/ not found" >&2; exit 1; }
...
if [ "${CVC_PLATFORM}" != "macos" ]; then
  command -v patchelf >/dev/null 2>&1 || { echo "pillow-cp311: patchelf missing" >&2; exit 1; }
...
print("pillow-cp311 build + verification complete")
PYCHECK
