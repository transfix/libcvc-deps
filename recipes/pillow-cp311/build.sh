#!/usr/bin/env bash
# recipes/pillow-cp311/build.sh — build pillow 11.1.0 FROM SOURCE (generated).
#
...
echo "pillow-cp311: building with ${PY_EXE}"
...
[ -n "${WHEEL}" ] || { echo "pillow-cp311: no wheel produced under ${WHEELHOUSE}" >&2; exit 1; }
