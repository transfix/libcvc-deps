# recipes/itsdangerous-cp311/build.ps1 — build itsdangerous 2.2.0 FROM SOURCE (generated).
...
$py = Get-CvcPythonExe
Write-Output "itsdangerous-cp311: building with $py"
...
if ($LASTEXITCODE -ne 0) { throw "itsdangerous-cp311: pip wheel failed ($LASTEXITCODE)" }

$wheel = Get-ChildItem -Path $wheelhouse -Filter '*.whl' -File | Select-Object -First 1
if (-not $wheel) { throw "itsdangerous-cp311: no wheel produced under $wheelhouse" }
Write-Output "itsdangerous-cp311: built $($wheel.Name)"

& $py -m pip install --no-index --no-deps --no-compile --ignore-installed `
    --prefix $env:CVC_INSTALL_DIR $wheel.FullName
if ($LASTEXITCODE -ne 0) { throw "itsdangerous-cp311: pip install failed ($LASTEXITCODE)" }
