# recipes/pygments-cp312/build.ps1 — build pygments 2.20.0 FROM SOURCE (generated).
...
$py = Get-CvcPythonExe
Write-Output "pygments-cp312: building with $py"
...
if ($LASTEXITCODE -ne 0) { throw "pygments-cp312: pip wheel failed ($LASTEXITCODE)" }

$wheel = Get-ChildItem -Path $wheelhouse -Filter '*.whl' -File | Select-Object -First 1
if (-not $wheel) { throw "pygments-cp312: no wheel produced under $wheelhouse" }
Write-Output "pygments-cp312: built $($wheel.Name)"

& $py -m pip install --no-index --no-deps --no-compile --ignore-installed `
    --prefix $env:CVC_INSTALL_DIR $wheel.FullName
if ($LASTEXITCODE -ne 0) { throw "pygments-cp312: pip install failed ($LASTEXITCODE)" }
