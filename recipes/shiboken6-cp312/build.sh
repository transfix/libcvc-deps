...
# pinned Qt-for-Python pyside-setup 6.8.2 tarball, against the cvcpkg python312
...
# the importable cp312 `shiboken6` package (wrapInstance), and the Shiboken6
...
# ── Locate the cvcpkg python3.12 interpreter in the dependency closure ───────
# (same pattern as recipes/vtk-python-cp312/build.sh) so the bindings + module
# are built against the SAME cp312 the host embeds.
PY_EXE=""
...
if [[ -z "${PY_EXE}" ]]; then
    echo "shiboken6: could not find python3.12 in the dependency closure" >&2
    exit 1
fi
...
# python312 interpreter's own prefix). shiboken otherwise prefixes
# CMAKE_INSTALL_PREFIX itself, but we pin it explicitly for determinism.
SITE_PACKAGES="${CVC_INSTALL_DIR}/lib/python3.12/site-packages"
...
# Per-interpreter column: the built shiboken6 tree stays in python3.12's
