# recipes/sympy-cp312/build.ps1 — build sympy 1.13.3 FROM SOURCE (generated).
...
$py = Get-CvcPythonExe
Write-Output "sympy-cp312: building with $py"
...
if ($LASTEXITCODE -ne 0) { throw "sympy-cp312: pip wheel failed ($LASTEXITCODE)" }

$wheel = Get-ChildItem -Path $wheelhouse -Filter '*.whl' -File | Select-Object -First 1
if (-not $wheel) { throw "sympy-cp312: no wheel produced under $wheelhouse" }
Write-Output "sympy-cp312: built $($wheel.Name)"

& $py -m pip install --no-index --no-deps --no-compile --ignore-installed `
    --prefix $env:CVC_INSTALL_DIR $wheel.FullName
if ($LASTEXITCODE -ne 0) { throw "sympy-cp312: pip install failed ($LASTEXITCODE)" }
