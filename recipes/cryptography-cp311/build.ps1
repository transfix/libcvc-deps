# recipes/cryptography-cp311/build.ps1 — Windows from-source build of
# cryptography 48.0.0 for the cp311 interpreter column.
#
...
Write-Output "cryptography-cp311: building with $py"
...
if ($LASTEXITCODE -ne 0) { throw "cryptography-cp311: build backend not importable from the build prefix" }
...
if (-not $maturinCmd) { throw "cryptography-cp311: no 'maturin' executable on PATH (maturin-cp311 supplies it in $bld\Scripts)" }
& $maturinCmd.Source --version
if ($LASTEXITCODE -ne 0) { throw "cryptography-cp311: staged maturin failed to run" }
...
function Resolve-CvcPrefixTool([string]$Name) {
    ...
    if (-not $cmd) { throw "cryptography-cp311: $Name not on PATH — is the 'rust' build dep in the closure?" }
    $path = $cmd.Source
    if (-not ($path.StartsWith($bld, 'OrdinalIgnoreCase') -or $path.StartsWith($deps, 'OrdinalIgnoreCase'))) {
        throw "cryptography-cp311: refusing $Name at $path — outside the cvcpkg prefixes ($bld, $deps)"
    }
    return $path
}
...
if ($env:CVC_CARGO_OFFLINE) {
    ...
    Write-Output "cryptography-cp311: CARGO_NET_OFFLINE=true — $env:CARGO_HOME must already hold every crate in Cargo.lock"
} else {
    $env:CARGO_NET_OFFLINE = 'false'
    Write-Output "cryptography-cp311: NOTE - cargo will FETCH crates.io (sdist ships Cargo.lock, no vendored sources)"
}
...
if (-not (Test-Path -LiteralPath $sslHeader)) {
    throw "cryptography-cp311: $sslHeader missing — is the openssl dep in the closure?"
}
...
Write-Output "cryptography-cp311: OPENSSL_DIR=$env:OPENSSL_DIR (version $env:CVC_OPENSSL_VERSION)"
...
& $py @pipArgs
if ($LASTEXITCODE -ne 0) { throw "cryptography-cp311: pip wheel failed ($LASTEXITCODE)" }

$wheel = Get-ChildItem -Path $wheelhouse -Filter '*.whl' -File | Select-Object -First 1
if (-not $wheel) { throw "cryptography-cp311: no wheel produced under $wheelhouse" }
Write-Output "cryptography-cp311: built $($wheel.Name)"
...
if ($LASTEXITCODE -ne 0) { throw "cryptography-cp311: pip install failed ($LASTEXITCODE)" }

$sitePackages = Join-Path $env:CVC_INSTALL_DIR 'Lib\site-packages'
if (-not (Test-Path -LiteralPath $sitePackages)) {
    throw "cryptography-cp311: no Lib\site-packages under $env:CVC_INSTALL_DIR after pip install"
}
Write-Output "cryptography-cp311: staged into $sitePackages"
...
'@
& $py -c $check
if ($LASTEXITCODE -ne 0) { throw "cryptography-cp311: verification failed ($LASTEXITCODE)" }

Write-Output "cryptography-cp311: build + verification complete"
