# recipes/idna-cp312/build.ps1 — build idna 3.16 FROM SOURCE (generated).
...
$py = Get-CvcPythonExe
Write-Output "idna-cp312: building with $py"
...
if ($LASTEXITCODE -ne 0) { throw "idna-cp312: pip wheel failed ($LASTEXITCODE)" }

$wheel = Get-ChildItem -Path $wheelhouse -Filter '*.whl' -File | Select-Object -First 1
if (-not $wheel) { throw "idna-cp312: no wheel produced under $wheelhouse" }
Write-Output "idna-cp312: built $($wheel.Name)"

& $py -m pip install --no-index --no-deps --no-compile --ignore-installed `
    --prefix $env:CVC_INSTALL_DIR $wheel.FullName
if ($LASTEXITCODE -ne 0) { throw "idna-cp312: pip install failed ($LASTEXITCODE)" }
