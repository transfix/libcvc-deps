#!/usr/bin/env bash
# recipes/contourpy-cp312/build.sh — build contourpy 1.3.1 FROM SOURCE
...
[ -n "${_PB11_PC}" ] || { echo "contourpy-cp312: python -m pybind11 --pkgconfigdir returned nothing" >&2; exit 1; }
...
[ -d "${_PB11_INC}" ] || { echo "contourpy-cp312: pybind11.get_include() -> ${_PB11_INC} does not exist" >&2; exit 1; }
export CXXFLAGS="-I${_PB11_INC} ${CXXFLAGS:-}"
echo "contourpy-cp312: pybind11 pc=${_PB11_PC} include=${_PB11_INC} cflags=[$(pkg-config --cflags pybind11 2>&1 || true)]"
