# recipes/cryptography-cp313/build.ps1 — Windows from-source build of
# cryptography 48.0.0 for the cp313 interpreter column.
#
...
Write-Output "cryptography-cp313: building with $py"
...
if ($LASTEXITCODE -ne 0) { throw "cryptography-cp313: build backend not importable from the build prefix" }
...
if (-not $maturinCmd) { throw "cryptography-cp313: no 'maturin' executable on PATH (maturin-cp313 supplies it in $bld\Scripts)" }
& $maturinCmd.Source --version
if ($LASTEXITCODE -ne 0) { throw "cryptography-cp313: staged maturin failed to run" }
...
function Resolve-CvcPrefixTool([string]$Name) {
    ...
    if (-not $cmd) { throw "cryptography-cp313: $Name not on PATH — is the 'rust' build dep in the closure?" }
    $path = $cmd.Source
    if (-not ($path.StartsWith($bld, 'OrdinalIgnoreCase') -or $path.StartsWith($deps, 'OrdinalIgnoreCase'))) {
        throw "cryptography-cp313: refusing $Name at $path — outside the cvcpkg prefixes ($bld, $deps)"
    }
    return $path
}
...
if ($env:CVC_CARGO_OFFLINE) {
    ...
    Write-Output "cryptography-cp313: CARGO_NET_OFFLINE=true — $env:CARGO_HOME must already hold every crate in Cargo.lock"
} else {
    $env:CARGO_NET_OFFLINE = 'false'
    Write-Output "cryptography-cp313: NOTE - cargo will FETCH crates.io (sdist ships Cargo.lock, no vendored sources)"
}
...
if (-not (Test-Path -LiteralPath $sslHeader)) {
    throw "cryptography-cp313: $sslHeader missing — is the openssl dep in the closure?"
}
...
Write-Output "cryptography-cp313: OPENSSL_DIR=$env:OPENSSL_DIR (version $env:CVC_OPENSSL_VERSION)"
...
& $py @pipArgs
if ($LASTEXITCODE -ne 0) { throw "cryptography-cp313: pip wheel failed ($LASTEXITCODE)" }

$wheel = Get-ChildItem -Path $wheelhouse -Filter '*.whl' -File | Select-Object -First 1
if (-not $wheel) { throw "cryptography-cp313: no wheel produced under $wheelhouse" }
Write-Output "cryptography-cp313: built $($wheel.Name)"
...
if ($LASTEXITCODE -ne 0) { throw "cryptography-cp313: pip install failed ($LASTEXITCODE)" }

$sitePackages = Join-Path $env:CVC_INSTALL_DIR 'Lib\site-packages'
if (-not (Test-Path -LiteralPath $sitePackages)) {
    throw "cryptography-cp313: no Lib\site-packages under $env:CVC_INSTALL_DIR after pip install"
}
Write-Output "cryptography-cp313: staged into $sitePackages"
...
'@
& $py -c $check
if ($LASTEXITCODE -ne 0) { throw "cryptography-cp313: verification failed ($LASTEXITCODE)" }

Write-Output "cryptography-cp313: build + verification complete"
