...
# Locate the cvcpkg python312 interpreter (cp312) in the dependency closure so the
...
foreach ($root in @($env:CVC_DEPS_PREFIX, $env:CVC_BUILD_PREFIX, $env:CVC_INSTALL_DIR)) {
    ...
    foreach ($cand in @("$root\python.exe", "$root\bin\python.exe", "$root\python3.12.exe")) {
        if (Test-Path $cand) { $pyExe = $cand; break }
    }
    if ($pyExe) { break }
}
if (-not $pyExe) { throw "vtk-python: could not find python (3.12) in the dependency closure" }
