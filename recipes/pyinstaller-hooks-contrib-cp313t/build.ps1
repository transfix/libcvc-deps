# recipes/pyinstaller-hooks-contrib-cp313t/build.ps1 — build pyinstaller-hooks-contrib 2026.6 FROM SOURCE (generated).
...
$py = Get-CvcPythonExe
Write-Output "pyinstaller-hooks-contrib-cp313t: building with $py"
...
if ($LASTEXITCODE -ne 0) { throw "pyinstaller-hooks-contrib-cp313t: pip wheel failed ($LASTEXITCODE)" }

$wheel = Get-ChildItem -Path $wheelhouse -Filter '*.whl' -File | Select-Object -First 1
if (-not $wheel) { throw "pyinstaller-hooks-contrib-cp313t: no wheel produced under $wheelhouse" }
Write-Output "pyinstaller-hooks-contrib-cp313t: built $($wheel.Name)"

& $py -m pip install --no-index --no-deps --no-compile --ignore-installed `
    --prefix $env:CVC_INSTALL_DIR $wheel.FullName
if ($LASTEXITCODE -ne 0) { throw "pyinstaller-hooks-contrib-cp313t: pip install failed ($LASTEXITCODE)" }

# The check must exercise the runtime closure, not the build-only backend.
$env:PYTHONPATH = ''
Invoke-CvcPythonCheck 'import _pyinstaller_hooks_contrib'
