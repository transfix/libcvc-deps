...
That is the whole story as of **cvcpkg 2.0.2**.  No `--no-deps`, no flags, no
Rust toolchain, nothing to compile: cvcpkg's core install is `click` +
`PyYAML`, and HaikuPorts ships both.

It did not use to be.  The folklore answer — *no, HaikuPorts has no pip and
pins `cryptography` at 3.4.8 against cvcpkg's `>=41.0` floor* — was about the
**declared** dependency list, never about what the code actually loads, and
that list has since been fixed (see [Ranked options](#ranked-options) below
for what changed and what the older releases need).
...
Verified in a clean virtualenv containing **only** `click==8.1.3` (the exact
version HaikuPorts ships) and `PyYAML` — first with cvcpkg installed
`--no-deps`, and since 2.0.2 with a plain `pip install`, which now resolves to
those same two packages:

```
$ cvcpkg install zlib bzip2 --prefix /tmp/p        # → 2 components installed
$ cvcpkg build zlib                                # → configure, build, smoke test pass
$ cvcpkg haiku draft-recipe zlib                   # → draft on stdout
```

So the blocker was **packaging metadata, not code**.  Up to cvcpkg 2.0.1,
`pyproject.toml` listed `cryptography`, `sqlalchemy`, `greenlet` and `httpx`
as mandatory rather than as extras, so `pip install cvcpkg` insisted on four
packages the client never imports.  None of them is satisfiable from the ports
tree: HaikuPorts has `cryptography` 3.4.8 (cvcpkg wanted `>=41`, which needs a
Rust toolchain) and `sqlalchemy` 1.3.24 (cvcpkg wants `^2.0`), and has no
`greenlet` or `httpx` port at all — `greenlet` being a C extension with
hand-written stack-switching assembly, not a wheel you can drop in.

Since 2.0.2 those four are extras, keyed to the role that needs them
(`[remote]` / `[publish]` / `[builder]` for httpx, `[signing]` for
cryptography, `[server]` for sqlalchemy + greenlet), and the mandatory list is
the two packages above.  Reaching for one you did not install is a one-line
error naming the exact `pip install` to run, not a traceback — so a Haiku user
can go as far as the ports tree allows and be told plainly where it stops.

### Ranked options

**1. `pip install cvcpkg` — works today, no flags.**  Contrary to the
...
pip resolves `click` and `PyYAML` — and finds HaikuPorts' copies already
installed, so nothing is built.  You do not get `--require-signatures` or the
`cvcpkg-server` commands, because their extras (`[signing]`, `[server]`) are
not installable here; asking for one says so in a single line.  Everything on
the client path works.

*On cvcpkg 2.0.1 and older*, add `--no-deps` — that is what keeps pip from
trying to compile `cryptography`, and the result is the same working client.
...
**2. Extras keyed to roles — done in 2.0.2.**  `cryptography`, `sqlalchemy`,
`greenlet` and `httpx` moved out of `[tool.poetry.dependencies]` into extras
(`signing`, `server`, `remote`/`publish`/`builder`), leaving `click` + `PyYAML`
as the base install.  `jsonschema` — never declared as a runtime dependency in
the first place, only as a dev one — became the `validate` extra on the same
terms, since `cvcpkg validate` is a recipe-maintainer command and no install
path reads a schema.  This is what makes option (1) a plain `pip install`.
Nothing in the client path broke: the same clean-venv trace that produced the
table above is the evidence, and every guarded import now reports the extra to
install rather than a `ModuleNotFoundError`.

**3. Unpack the source tree onto `PYTHONPATH` — also fixed in 2.0.2.**  The
no-pip route used to fail on import:
...
`__version__` was read from installed distribution metadata with no fallback,
so cvcpkg could not be imported from a bare source checkout.  It now falls back
to a version constant when the metadata is absent, so
`PYTHONPATH=/path/to/libcvc-deps/src python3.10 -m cvcpkg` works — useful when
you want a git checkout rather than a release, and the only route that needs no
pip at all.
...
and the one that would make `pkgman install cvcpkg` work.  It used to be
blocked on (2) — a recipe that has to carry `cryptography >= 41` is not
portable — and is now unblocked: the `REQUIRES` list is `python3.10`, `click`
and `pyyaml`.  Nobody has written it yet.  If you want to, `cvcpkg haiku
draft-recipe` will not help: cvcpkg has no recipe for itself.
...
Haiku toolchain — but per the section above it is a convenience, not a
necessity: cvcpkg runs on Haiku, so a Haiku box can also just build for
itself.
